...
export class RandomStrategy {
   ...
   * Select an action type based on weights
   */
  private selectActionType(): string {
   ...
   */
  private selectRandomElement(elements: UIElement[]): UIElement | null {
  ...
  private createSwipeAction(direction: string): Action {
    ...
    return {
      id: uuidv4(),
      type: 'swipe',
      direction: direction.replace('swipe', '').toLowerCase() as 'up' | 'down' | 'left' | 'right',
      x: startX,
      y: startY,
      timestamp: Date.now(),
    };
