...
import type { ScreenState, UIElement, Action, AutomationConfig, StrategyConfig } from '../types.js';
...
export class AIExplorer {
  private strategy: RandomStrategy | BFSStrategy | DFSStrategy | SmartStrategy;
  private currentStrategy: 'random' | 'bfs' | 'dfs' | 'smart';
  private consecutiveRandom: number = 0;
  private maxConsecutiveRandom: number;
  private visitedScreens: Set<string> = new Set();
  private screenElements: Map<string, UIElement[]> = new Map();
  private elementPriorities: Record<string, number>;
  ...
  private createStrategy(type: 'random' | 'bfs' | 'dfs' | 'smart'): 
    RandomStrategy | BFSStrategy | DFSStrategy | SmartStrategy {
  ...
  decide(screen: ScreenState): Action {
    ...
    if (this.currentStrategy === 'smart') {
      const smartStrategy = this.strategy as SmartStrategy;
  ...
  private hashScreen(screen: ScreenState): string {
    ...
    const elementSignature = screen.elements
      .map((e: UIElement) => `${e.type}:${e.label || ''}`)
      .sort()
      .join('|');
   ...
   */
  private createBackAction(): Action {
    return {
      id: uuidv4(),
      type: 'back',
      timestamp: Date.now(),
